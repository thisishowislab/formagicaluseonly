...
const tiers = [{
  ...
  icon: Heart,
  color: 'hsl(185 80% 50%)',
  benefits: ['Your name in the Book of Wanderers', 'Monthly desert dispatch newsletter', 'Access to community Discord']
}, {
  id: 'seeker',
  name: 'Seeker',
  amount: 50,
  icon: Star,
  color: 'hsl(45 80% 55%)',
  benefits: ['All Wanderer benefits', 'Early access to new realm openings', '10% off all CSC items', 'Quarterly video updates from the bus']
}, {
  id: 'guardian',
  name: 'Guardian',
  amount: 100,
  icon: Gem,
  color: 'hsl(280 70% 60%)',
  benefits: ['All Seeker benefits', 'Exclusive Guardian-only merch annually', 'Your name etched on the Manifestorium wall', 'Direct input on realm development']
}, {
  id: 'keeper',
  name: 'Realm Keeper',
  amount: 250,
  icon: Crown,
  color: 'hsl(320 70% 55%)',
  benefits: ['All Guardian benefits', 'Annual care package from the desert', 'Private virtual tour of new creations', 'Lifetime 20% off all items', 'A portal named in your honor']
}];
const wishlistItems = [{
  icon: Droplets,
  name: 'Water & Supplies',
  desc: 'Clean water, non-perishables, batteries'
}, {
  icon: Wrench,
  name: 'Tools & Materials',
  desc: 'Hand tools, lumber, fasteners, solar equipment'
}, {
  icon: Tent,
  name: 'Camping Gear',
  desc: 'Tarps, shade structures, coolers'
}, {
  icon: Gift,
  name: 'Art Supplies',
  desc: 'Paint, welding rods, scrap metal, fabric'
}];
const TheOffering = () => {
      ...
      <main className="relative z-10 pt-24 pb-16 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          duration: 0.8
            ...
            <p className="text-sm tracking-[0.3em] uppercase text-muted-foreground mb-4 font-body">SUPPORT THE MAGIC</p>
            <h1 className="text-4xl md:text-6xl font-display font-medium tracking-wider text-foreground mb-6 text-glow-primary">
              The Offering
            </h1>
            <p className="text-lg text-muted-foreground font-body italic max-w-2xl mx-auto leading-relaxed">Why Support Us?
Every 3D print, every laser cut, every experimental installation runs on materials, electricity, and the sheer will to keep making weird things in the desert.


Your support keeps the machines humming and the ideas flowing. Think of it as funding the most interesting rest stop on your way through the desert.</p>
          </motion.header>
              ...
              scale: 1.02
                ...
                background: `radial-gradient(ellipse at center, ${tier.color.replace(')', ' / 0.15)')} 0%, transparent 70%)`
                  ...
                  boxShadow: `0 0 20px ${tier.color.replace(')', ' / 0.3)')}`
                      ...
                      <tier.icon className="w-6 h-6" style={{
                    color: tier.color
                  }} />
                    </div>

                    {/* Name & Price */}
                    <h3 className="text-xl font-display font-medium text-center mb-2" style={{
                  color: tier.color
                }}>
                      {tier.name}
                    </h3>
                    ...
                    <ul className="space-y-2 flex-1">
                      {tier.benefits.map((benefit, i) => <li key={i} className="text-sm text-muted-foreground font-body flex items-start gap-2">
                          <span style={{
                      color: tier.color
                    }}>✦</span>
                          {benefit}
                    ...
                    {/* CTA */}
                    <motion.a href={`mailto:hello@manifestorium.com?subject=Become a ${tier.name}&body=I'd like to become a ${tier.name} ($${tier.amount}/month) supporter of The Manifestorium.`} whileHover={{
                  scale: 1.05
                }} whileTap={{
                  scale: 0.98
                }} className="mt-6 block w-full py-3 rounded text-center font-display tracking-widest text-sm uppercase transition-all duration-300" style={{
          ...
          delay: 0.6
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {wishlistItems.map((item, index) => <motion.div key={item.name} initial={{
              opacity: 0,
              scale: 0.9
            }} animate={{
              opacity: 1,
          ...
          delay: 0.9
            ...
            <p className="text-muted-foreground font-body">
              ...
              <a href="mailto:hello@manifestorium.com" className="text-primary hover:text-accent transition-colors">thisishowislab@gmail.com</a>
            </p>
          </motion.section>
        </div>
      </main>
